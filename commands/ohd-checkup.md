...
3b. **Retired-route pass (semantic)**: a PRESENT item can name a **route a
   ...
   yourself (that judgment happened at release time). For each relayed
   BEHAVIOR-CHANGE that retires or re-routes a mechanism, grep the project's
   CLAUDE.md **and its memory store** (`MEMORY.md` + `memory/*.md` — a
   standing rule there re-derives just as reliably, and no other pass reads
   them) for the retired mechanism; add one `RETIRED-ROUTE` row per hit,
   naming the superseding route. Skip when `harness-changes` is
   NO-BASELINE / ANOMALY / UNKNOWN (no bounded list ⇒ no pass).
...
5. **Per-item repair, gated**: AskUserQuestion per drifted item (batch
   ...
   - `RETIRED-ROUTE` → replace the named mechanism with the superseding route,
     in place, at every hit (CLAUDE.md and the memory store); KEEP the rule —
     the gate is not the thing that expired. A project may legitimately
     decline (a fork carrying its own route on purpose); record that and move on.
