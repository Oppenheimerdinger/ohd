...
- (스킬) `autonomous-mandate` — "자율진행"/"자율로 완수해" 순간 자동 발화: oh-my-claudecode의 `ralph` 무장이 첫 행동 — 상태를 명시적으로 기입한 뒤 반복 배너로 무장 여부를 확인 (스킬 호출만으로는 루프가 걸리지 않음). OMC가 없으면 내장 `/loop`으로 정직하게 강등 (맨몸 자율 세션의 침묵 정지 방지)
