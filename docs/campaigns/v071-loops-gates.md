...
- residual watch: row 0 is the only NAMED-contents row with no mechanical backstop (skill-carried by explicit tradeoff — a seeded prompt would read as owed on every land). First data point favorable (this land's own row 0 carries the citation) but it was written by the session that implemented the rule — the most favorable case; the residual stays open until a session that did NOT build it produces the citation unprompted.
- carry-forward (implementer, post-merge): the spec's 3c sequencing note is accurate-but-pessimistic — file removal on the default branch auto-delists the workflow, so the `gh workflow disable` step it names is unnecessary; amend on the next touch of that spec (not worth its own commit). The DRIFT→CONSISTENT transition was witnessed live end-to-end, the one thing fixtures could not supply.
