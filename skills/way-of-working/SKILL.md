...
| A review finding you have decided to act on | a backlog / follow-on LINE by default (`docs/backlog.md`, or the state doc's `- follow-on:` row) — promoting it to a CAMPAIGN requires naming, in that line, why it cannot wait. Every finding arrives with an implicit "so fix it", and findings about the harness become campaigns whose reviews produce more findings: a five-campaign field mandate stopped because a human asked why it was taking so long, not because any mechanism noticed |
